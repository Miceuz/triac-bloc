...
plt.axvline(np.mean(diff) + sigma, color="yellow")
plt.title("Pulse length differeces for adjacent pulses, R input = 16.4k R pullup = 10k")
plt.savefig("Pulse length differeces for adjacent pulses, R input = 16k4 R pullup = 10k")
plt.show()

#plt.plot(range(0, N), differential(np.abs(x[:,2])), color="red")
